	...
	function PeFieldImportDemo(target, conf) {
		...
		function start() {
			if (location.search.match(/imported=ok/)) {
				log("imported","<strong>New options loaded</strong>, you can now customize the theme<br><br><strong style=\"color: red\">IMPORTANT NOTES</strong><br><ol><li>If menus do not behave as expected, go to Appareance->Menus and click \"Save Menu\" for each menu in the theme.<li>The import plugin is not always able to preserve post IDs, as such some elements may not appear (for instance a video post or project would be missing the video). To fix, just edit the post (or project) with the missing video and click \"Update\" to save it.</ol>");
				button.hide();
			} else {
