...
(function ($) {
	...
	function PeFieldItems(target, conf) {
			...
			if (target.attr("data-legend") == "yes") {
				...
				for (i = 0;i<fields.length;i++) {
					...
					if (field.type != "hidden") {
						buffer += '<div style="width:%1px">%0</div>'.format(field.name,field.width);
					}
				}
				buffer += "</div>";
				legend = $(buffer);
		...
		function fixId(idx) {
			var i,el,els = tbody.find(this).find("input, select");
		...
		function addValue(value) {
			...
			for (i = 0;i<fields.length;i++) {
				field = fields[i];
				id = "%0_%1".format(field.name,uniqID);
				if (i === 0 && typeof value != "object") {
				...
				case "textimg":
					fieldTemplate = '<div><input id="pe_fields_item_field_%0" name="%1[%5][%2]" data-name="%2" type="text" value="%3" %4><a class="add-inline" title="Add Image" href=""></a></div>'.format(
						...
						name,
						field.name,
						fval,field.width ? 'style="float:left;width:%0px"'.format(field.width-20) : "",
						uniqID
					);
